...
class ParsedQuery(object):
    ...
    def __process_tokenlist(self, token_list):
        """
    ...
    def as_create_table(self, table_name, overwrite = False):
        ...
        sql = self.stripped()
        if overwrite:
            exec_sql = "DROP TABLE IF EXISTS %s;\n" % table_name
        exec_sql += "CREATE TABLE %s AS \n%s" % (table_name, sql)
        return exec_sql
    ...
    def get_query_with_new_limit(self, new_limit):
        ...
        """
        if not self._limit:
            return "%s\nLIMIT %s" % (self.stripped(), new_limit)
        limit_pos = None
        ...
        if limit.ttype == sqlparse.tokens.Literal.Number.Integer and new_limit < int(limit.value):
            limit.value = new_limit
        elif limit.is_group:
            limit.value = "%s, %s" % (next(limit.get_identifiers()), new_limit)
